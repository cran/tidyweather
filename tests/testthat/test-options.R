...
test_that("weather_options() can set and get frost_threshold", {
    ...
    weather_options(extreme.frost_threshold = -5)
    options <- weather_options()
    expect_equal(options$extreme$frost_threshold, -5)

    # Set to decimal value
    weather_options(extreme.frost_threshold = 1.5)
    options <- weather_options()
...
test_that("weather_options() handles multiple options at once", {
    ...
    weather_options(extreme.frost_threshold = 3)
...
test_that("weather_reset() resets options to defaults", {
    ...
    weather_options(extreme.frost_threshold = 10)
...
test_that("weather_options() preserves other settings when updating one", {
    ...
    # Get initial state
    initial_options <- weather_options()

    # Change frost_threshold
    weather_options(extreme.frost_threshold = 5)

    # Verify only frost_threshold changed
    new_options <- weather_options()
...
test_that("weather_options() works with different data types", {
    ...
    # Test with numeric
    weather_options(extreme.frost_threshold = 2.5)
    expect_equal(weather_options()$extreme$frost_threshold, 2.5)

    # Test with negative
    weather_options(extreme.frost_threshold = -1.5)
    expect_equal(weather_options()$extreme$frost_threshold, -1.5)
})

test_that("WEATHER_OPTIONS is not directly accessible", {
    # WEATHER_OPTIONS should not be accessible from global environment
    # Note: It might be exported during development but shouldn't be in production
    expect_true(is.function(weather_options))
})

test_that("options functions are properly exported", {
    # These functions should be available
    expect_true(exists("weather_options"))
    expect_true(exists("weather_reset"))
})
